...
namespace Jagat\Jax\Providers;


use Carbon\Carbon;
use Illuminate\Support\ServiceProvider;
use Laravel\Passport\Passport;
use Jagat\Jax\Console\InstallCommand;
use Route;

class JaxServiceProvider extends ServiceProvider
{
    /**
    ...
    public function boot()
    {
        if ($this->app->runningInConsole()) {
            ...
            $this->publishes([
                __DIR__.'/../../config/jax.php' => config_path('jax.php'),
            ], 'config');
    ...
    private function registerRouter()
    {
        ...
        Passport::tokensExpireIn(Carbon::now()->addDays(config('jax.passport_token_ttl')));

        Passport::refreshTokensExpireIn(Carbon::now()->addDays(config('jax.passport_refresh_token_ttl')));
    }
}